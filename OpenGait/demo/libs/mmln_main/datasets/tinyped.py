...
def val_aug(img, mask):
    ...
    assert img.shape[:2] == mask.shape[:2], f"Augmented image size {img.shape[:2]} does not match mask size {mask.shape[:2]}"
    return img, mask
...
class TinyDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if p_ratio > self.mosaic_ratio or self.mode == 'val' or self.mode == 'test':
            ...
            if self.transform:
                ...
                img, mask = np.array(img), np.array(mask)
            assert img.shape[:2] == mask.shape[:2], f"Transformed image size {img.shape[:2]} does not match mask size {mask.shape[:2]}"
    ...
    def load_img_and_mask(self, index):
        ...
        assert img.size == mask.size, f"Image size {img.size} does not match mask size {mask.size}"
        return img, mask
    ...
    def load_mosaic_img_and_mask(self, index):
        ...
        assert img.shape[:2] == mask.shape[:2], f"Concatenated image size {img.shape[:2]} does not match mask size {mask.shape[:2]}"
...
def show_mask(img, mask, img_id):
    ...
    mask = np.array(mask.convert('RGB'))
    ax1.imshow(img)
    ax1.set_title('RS IMAGE ' + str(img_id)+'.jpg')
    ax2.imshow(mask)
