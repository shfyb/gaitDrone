...
def seg_image(img, seg_config, seg_checkpoint, save_name, savesil_path):
    """
     参数：
        img: 输入图像 (numpy数组，BGR格式)
        config: 保留参数（兼容原调用，实际不使用）
        save_name: 保存文件名
        savesil_path: 保存路径
    """
    
    seg_model = init_segmentor(seg_config, seg_checkpoint, device='cuda:0')
    
    # 创建保存目录
    os.makedirs(savesil_path, exist_ok=True)
    
    # 执行推理
    result = inference_segmentor(seg_model, img)
    
    # 获取分割掩码（假设类别0是背景，1是前景）
    seg_mask = result[0].astype(np.uint8)

     # 调试输出：打印检测到的类别
    unique_classes = np.unique(seg_mask)
    print(f"检测到类别ID: {unique_classes}")
    
    # 转换为二值掩膜（根据你的需求调整类别索引）
    # 这里假设person类别是1（具体根据模型定义调整）

    person_class = 1

    out_mask = np.where(seg_mask == person_class, 255, 0).astype(np.uint8)
    
    # 可选：保持原始阈值处理逻辑（根据实际需求决定是否需要）
    # therehold = 80
    # out_mask = np.where(out_mask < therehold, 0, 255)
