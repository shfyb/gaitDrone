...
from mmln_main.seg_demo_mmln import seg_image
...
config = './demo/libs/mmln_main/mmln.py'  # 替换为你的配置文件路径
checkpoint = './demo/checkpoints/MMLNseg/uavid666-v2.ckpt'  # 替换为你的模型权重路径
        ...
        if ret_val:
            if frame_id in ids and frame_id%4==0:
                ...
                for tidxywh in track_result[frame_id]:
                    ...
                    #调用 seg_image 函数进行分割
                    seg_image(tmp, config, checkpoint,  save_name, savesil_path)
