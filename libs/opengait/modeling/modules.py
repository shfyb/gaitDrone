...
class GradualWarmupScheduler(_LRScheduler):
    def __init__(self, optimizer, multiplier, total_iteration, after_scheduler=None):
    ...
    def get_lr(self):
        ...
        if self._step_count <= self.total_iteration:
            return [base_lr * (self._step_count / self.total_iteration) for base_lr in self.base_lrs]
        if self.after_scheduler:
            if not self.finished:
                self.after_scheduler.base_lrs = self.base_lrs
                self.finished = True
            self.after_scheduler.last_epoch = self._step_count
...
class SetBlockWrapper_3D(nn.Module):
    def __init__(self, forward_block):
        super(SetBlockWrapper, self).__init__()
        self.forward_block = forward_block
