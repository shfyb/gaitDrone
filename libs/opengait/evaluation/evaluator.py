...
from .metric import mean_iou, cuda_dist, compute_ACC_mAP, evaluate_rank, evaluate_many, compute_mAP
...
def single_view_gallery_evaluation(feature, label, seq_type, view, dataset, metric):
    probe_seq_dict = {'CASIA-B': {'NM': ['nm-03', 'nm-04'], 'BG': ['bg-01', 'bg-02'], 'CL': ['cl-01', 'cl-02']},
                      'OUMVLP': {'NM': ['00']},
    ...
    gallery_seq_dict = {'CASIA-B': ['nm-01', 'nm-02'],
                        'OUMVLP': ['01'],
    ...
    if dataset == 'SUSTech1K':
        num_rank = 5 
    view_num = len(view_list)  # view_num = 10 
    map = {}
    for (type_, probe_seq) in probe_seq_dict[dataset].items():  # first is nm-03 nm-04
        map[type_] = np.zeros(view_num) - 1.
        acc[type_] = np.zeros((view_num, view_num, num_rank)) - 1.  # acc['nm']  is 10*10 matrix
        for (v1, probe_view) in enumerate(view_list):   # 遍历 1-10 view 
            pseq_mask = np.isin(seq_type, probe_seq) & np.isin(
                view, probe_view)     # seq_type 表示所有的 nm/bg/cl标签， probe_seq表示 当前probe的label view 表示所有数据的视角， 该目标就是找到哪些是probe
            ...
            probe_x = feature[pseq_mask, :]
            probe_y = label[pseq_mask]   #这里就找到了probe x和probe y 

            for (v2, gallery_view) in enumerate(view_list):
                gseq_mask = np.isin(seq_type, gallery_seq_dict[dataset]) & np.isin(
                    view, [gallery_view])   # 这一步是找到 gallery 的mask 如果这个是gallery 那么它就是true
                ...
                gallery_y = label[gseq_mask]
                gallery_x = feature[gseq_mask, :]  #找到了gallery x 和gallery y 
                dist = cuda_dist(probe_x, gallery_x, metric)
                idx = dist.topk(20, largest=False)[1].cpu().numpy()
                # 假设 probe_y 是探针的标签数组，将其重塑为列向量
                probe_labels_col = np.reshape(probe_y, [-1, 1])

                # 比较探针标签和画廊中最匹配项的标签是否相同，结果是一个布尔矩阵
                matches = probe_labels_col == gallery_y[idx[:, 0:num_rank]]

                # 计算累积和，用于确定每个探针在前N个排名中是否有匹配
                cumulative_matches = np.cumsum(matches, axis=1)

                # 将累积匹配转换为布尔值（大于0表示至少有一个匹配）
                match_flags = cumulative_matches > 0

                # 对每个探针，计算在前N个排名中有匹配的次数，并转换为百分比
                match_counts = np.sum(match_flags, axis=0)
                accuracy_percentages = np.round(match_counts * 100 / dist.shape[0], 2)

                # 将计算得到的百分比赋值给相应的acc数组位置
                acc[type_][v1, v2, :] = accuracy_percentages
            
            gallery_all_angle_mask = np.isin(seq_type,gallery_seq_dict[dataset])
            gallery_y_all_angle = label[gallery_all_angle_mask]
            gallery_x_all_angle = feature[gallery_all_angle_mask, :]
            dist = cuda_dist(probe_x, gallery_x_all_angle, metric)
            
            map_single_probe_view = compute_mAP(dist.cpu().numpy(), probe_y, gallery_y_all_angle,view[pseq_mask],view[gallery_all_angle_mask])
            map[type_][v1] = map_single_probe_view

                # average_map = []
                # for i in range(len(probe_y)):
                #     # 获取当前探针的真实标签
                #     true_label = probe_y[i]

                #     # 获取画廊中与当前探针相匹配的序列的索引
                #     sorted_gallery_idx = idx[i, :]

                #     # 获取这些序列的真实标签
                #     sorted_gallery_labels = gallery_y[sorted_gallery_idx]

                #     # 计算是否匹配的布尔数组
                #     matches = (sorted_gallery_labels == true_label)
                ...
                #     # 只考虑正确匹配的位置的精度
                #     relevant_precisions = precisions[matches]
                ...
                #     else:
                #         AP = 0
                #     average_map.append(AP)
                # MAP = np.mean(average_map)
                # map[type_][v1, v2, :] = MAP
                # # acc[type_][v1, v2, :] = np.round(np.sum(np.cumsum(np.reshape(probe_y, [-1, 1]) == gallery_y[idx[:, 0:num_rank]], 1) > 0, 0) * 100 / dist.shape[0], 2)
    ...
    msg_mgr.log_info('===mAP (Exclude identical-view cases)===')
    out_str = ""
    
    for type_ in probe_seq_dict[dataset].keys():
        msg_mgr.log_info(f'{type_}@mAP: {map[type_]}')
        result_dict[f'scalar/test_accuracy/{type_}@mAP'] = np.mean(map[type_])
        out_str += f"{type_}@mAP: {np.mean(map[type_]):.2f}%\t"
    msg_mgr.log_info(out_str)

    # for rank in range(num_rank):
    #     out_str = ""
    #     for type_ in probe_seq_dict[dataset].keys():
    #         sub_map = de_diag(map[type_][:,:,rank], each_angle=True)
    #         if rank == 0:
    #             msg_mgr.log_info(f'{type_}@mAP: {sub_map}')
    #             result_dict[f'scalar/test_accuracy/{type_}@mAP'] = np.mean(sub_map)
    #         out_str += f"{type_}@R{rank+1}: {np.mean(sub_map):.2f}%\t"
    #     msg_mgr.log_info(out_str)
    return result_dict



# def evaluate_drones(feature, label, seq_type, view, dataset, metric):
#     probe_seq_dict = {'DroneGait2': {'NM': ['nm-03', 'nm-04'], 'BG': ['bg-01', 'bg-02'], 'CL': ['cl-01', 'cl-02']}}
#     gallery_seq_dict = {'CASIA-B': ['nm-01', 'nm-02']}
#     msg_mgr = get_msg_mgr()
#     acc = {}
#     view_list = sorted(np.unique(view))
...
#     map = {}
#     for (type_, probe_seq) in probe_seq_dict[dataset].items():  # first is nm-03 nm-04
#         map[type_] = np.zeros((view_num, view_num, num_rank)) - 1.
#         acc[type_] = np.zeros((view_num, view_num, num_rank)) - 1.  # acc['nm']  is 10*10 matrix
#         for (v1, probe_view) in enumerate(view_list):   # 遍历 1-10 view 
#             pseq_mask = np.isin(seq_type, probe_seq) & np.isin(
#                 view, probe_view)     # seq_type 表示所有的 nm/bg/cl标签， probe_seq表示 当前probe的label view 表示所有数据的视角， 该目标就是找到哪些是probe
...
#                 # 将计算得到的百分比赋值给相应的acc数组位置
#                 acc[type_][v1, v2, :] = accuracy_percentages
#             # all_angle_mask = np.isin(seq_type,gallery_seq_dict[dataset])
#             # gallery_y_all_angle = label[all_angle_mask]
#             # gallery_x_all_angle = feature[all_angle_mask, :]
#             # dist = cuda_dist(probe_x, gallery_x_all_angle, metric)
#             # idx = dist.topk(20, largest=False)[1].cpu().numpy()
#                 # average_map = []
#                 # for i in range(len(probe_y)):
