...
def compute_mAP(distmat, q_pids, g_pids, q_views=None, g_views=None):
    ...
    for q_idx in range(num_q):
        q_idx_dist = distmat[q_idx]   # 遍历 query  获取query对全部gallery 的dist
        q_idx_glabels = g_pids         #获取全部gallery的label 
        if q_views is not None and g_views is not None:
            q_idx_mask = np.isin(g_views, q_views[q_idx], invert=True) | np.isin(    
                g_pids, q_pids[q_idx], invert=True)  # 找到和probe的view视角不相同的gallery  标记为true  然后找到与probe的label不相同的gallery 标记为true?  # 只去掉与自己label相同view 的gallery
            q_idx_dist = q_idx_dist[q_idx_mask]   # 获得gallery对应的dist
            q_idx_glabels = q_idx_glabels[q_idx_mask]  #获得对应的label
